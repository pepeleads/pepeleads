            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0 font-size-18">All Offers</h4>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const entriesToShow = 15000; // Change this value as per your requirement
        ...
        for (let i = offers.length-1; i > 0 ; i--) {
            ...
            let countries=(offer?.countries).split('|');
            let categories=(offer?.categories).split('|');
            row.innerHTML = `
                <td>
                    <input type="checkbox" class="form-check-input check-items" name="id[]" value="${offer?.id}">
                </td>
                <td>${offer?.id}</td>
                <td><img src="${offer?.image_url}" height="18"/></td>
                <td><a href="${(offer?.preview_url)? offer?.preview_url: '#'}" ${offer?.preview_url ? '' : `onclick="alert('No Preview Found')"`} onclick><button class="btn btn-warning d-flex"><i class=" fas fa-eye m-1"></i>Preview Offer</button></a></td>
                <td class="pl-0"><a href="#" class="m-0 p-0">${offer?.name}</a></td>
                <td>$${parseFloat((offer?.credit*offer?.rate?.rate?.network_rate)/100).toFixed(2)}</td>
                <td><a href="/request/offer/${btoa(offer?.id)}"  target="_blank"><button class="btn btn-warning">Request Offer</button></a></td>
